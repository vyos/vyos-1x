...
def is_addr_assigned(
    ...
    if allow_nonlocal:
        if is_ipv4(ip_address) and \
           sysctl_read(['net', 'ipv4', 'ip_nonlocal_bind']) == '1':
            return True
        if is_ipv6(ip_address) and \
...
def is_ipv4_range(addr: str) -> bool:
    ...
    try:
        return ip_address(start).version == 4 and ip_address(end).version == 4
    except ValueError:
        return False
...
def is_ipv6_range(addr: str) -> bool:
    ...
    start, end = addr.split('-')
    try:
        return ip_address(start).version == 6 and ip_address(end).version == 6
    except ValueError:
        return False
