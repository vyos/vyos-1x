...
class TestMonitoringTelegraf(VyOSUnitTestSHIM.TestCase):
    ...
    def test_08_agent_configuration(self):
        """
        ...
        # Make sure max value is not broken
        max_ns = 9223372036854775808

        self.cli_set(base_path + ['agent', 'collection-jitter', f"{max_ns}ns"])
        with self.assertRaises(ConfigSessionError):
            self.cli_commit()
        self.cli_discard()

        self.cli_set(base_path + ['agent', 'flush-interval', f"{max_ns}ns"])
        with self.assertRaises(ConfigSessionError):
            self.cli_commit()
        self.cli_discard()

        self.cli_set(base_path + ['agent', 'flush-jitter', f"{max_ns}ns"])
        with self.assertRaises(ConfigSessionError):
            self.cli_commit()
        self.cli_discard()

        self.cli_set(base_path + ['agent', 'interval', f"{max_ns}ns"])
        with self.assertRaises(ConfigSessionError):
            self.cli_commit()
        self.cli_discard()
