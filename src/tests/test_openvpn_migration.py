...
class TestOpenVPNMigration(TestCase):
    ...
    def test_raw_dev_type_drops_dco(self):
        # a raw "dev-type" overrides the one rendered from the CLI
        config = config_tree(
    ...
    def test_raw_afunix_dev_node_drops_dco(self):
        config = config_tree(
            '        mode server\n'
    ...
    def test_raw_real_dev_node_keeps_dco(self):
        # only an AF_UNIX node rules out the offload
        config = config_tree(
    ...
    def test_raw_topology_drops_dco_in_server_mode(self):
        config = config_tree(
    ...
    def test_raw_topology_ignored_outside_server_mode(self):
        config = config_tree(
            '        mode client\n        openvpn-option "--topology net30"\n'
        )
        self.migrate(config)
