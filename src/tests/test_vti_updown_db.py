    ...
    def test_duplicate_entries_survive_a_reload(self):
        ...
        with patch('vyos.utils.vti_updown_db.Lock', MagicMock()):
            with open_vti_updown_db_for_create_or_update() as db:
                db.add('vti0', 'conn-a', 'IKEv2')
                db.add('vti0', 'conn-a', 'IKEv2')
                # Persist via the same write path commit() uses, without its
                # interface bring-up/down side effects, which this class does
                # not mock.
                db._fileHandle.seek(0)
                db._fileHandle.write(' '.join(db._ifspecs))
                db._fileHandle.truncate()
    ...
    def test_persistent_entries_are_not_counted(self):
        ...
        with patch('vyos.utils.vti_updown_db.Lock', MagicMock()):
            with open_vti_updown_db_for_create_or_update() as db:
                db.add('vti0')
                db.add('vti0')
                self.assertEqual(db._ifspecs.count('vti0'), 1)
                db.remove('vti0')
                self.assertFalse(db.wantsInterfaceUp('vti0'))
