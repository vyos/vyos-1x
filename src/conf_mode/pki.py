...
from vyos.pki import get_acme_chain_base64
...
def acme_chain_needs_update(pki: dict, cert: str, cert_chain_base64: str) -> bool:
    """Return True if AUTOCHAIN_<cert> should be (re-)set to
    cert_chain_base64: no manually-added CA certificate already covers
    this chain, and the currently stored AUTOCHAIN_<cert> value (if any)
    differs from it.
    """
    if 'ca' in pki:
        for ca_base64, cli_path in dict_search_recursive(pki['ca'], 'certificate'):
            # Ignore automatically added CA certificates - only a manually
            # added one counts as "already present" here
            if any(item.startswith(AUTOCHAIN_PREFIX) for item in cli_path):
                continue
            if cert_chain_base64 == ca_base64:
                return False

    current = dict_search_args(pki, 'ca', f'{AUTOCHAIN_PREFIX}{cert}', 'certificate')
    return not bool(current) or current != cert_chain_base64
...
def get_config(config=None):
    ...
    # see op_mode/pki.py's renew_certbot()) shows up above as a change
    # under 'ca', but dependents (haproxy, https, ...) are only registered
    # below on a change to 'certificate' (or an explicit 'ca_certificate'
    # reference, which these auto-imported CAs never have). Temporarily
    # fold a changed AUTOCHAIN_<cert> into 'changed.certificate' for just
    # the dependency-registration loop below, then restore it further
    # down (search for "restore 'changed.certificate'"). It must not
    # reach generate()'s own "certificate config changed, delete and
    # re-request via ACME" check further down in this same pki dict: that
    # check has no reason to run just because the CA changed, and doing
    # so anyway deletes and re-requests the certificate for no reason -
    # burning an ACME issuance (and, at volume, tripping rate limits) on
    # every AUTOCHAIN-only change, confirmed live.
    original_changed_certificate = set(pki.get('changed', {}).get('certificate', []))
    changed_ca = pki.get('changed', {}).get('ca', [])
    if changed_ca:
        changed_certificate = set(original_changed_certificate)
        for ca_name in changed_ca:
            if ca_name.startswith(AUTOCHAIN_PREFIX):
                changed_certificate.add(ca_name[len(AUTOCHAIN_PREFIX):])
        if changed_certificate != original_changed_certificate:
            dict_set_nested('changed.certificate', changed_certificate, pki)
    ...
    # restore 'changed.certificate' to its pre-bridge value now that
    # dependents have been registered above using the (possibly bridged)
    # one - see the comment where original_changed_certificate was saved.
    if changed_ca:
        dict_set_nested('changed.certificate', original_changed_certificate, pki)
    ...
    # Attach the Config instance so generate() can pass it to add_cli_node()
    # for the AUTOCHAIN import, keeping this same Config's in-memory tree in
    # sync for any dependent invoked later in this commit - see
    # Config.sync_local_set()'s docstring for why that matters.
    pki.conf = conf
...
def generate(pki):
    ...
    # Certbot renewal needs to re-trigger the services to load up the new
    # PEM file. Do not return here - fall through so a renewed cert's
    # chain.pem can still be (re-)imported into pki ca AUTOCHAIN_<cert>
    # below, in case the intermediate CA was missing or has changed.
    is_certbot_renew = 'certbot_renew' in pki
    if is_certbot_renew:
        force = 'force' in pki['certbot_renew']
        certbot_renew(config=pki, force=force)
    ...
    if 'certificate' in pki:
        ...
        for name, cert_conf in pki['certificate'].items():
            if 'acme' in cert_conf:
                ...
                # something changed in the configuration. Skip this on a
                # certbot_renew pass: certbot_renew() above already renewed
    ...
    if os.path.exists(f'{vyos_certbot_dir}/live'):
        for cert in certbot_list_on_disk:
            ...
            if acme_chain_needs_update(pki, cert, cert_chain_base64):
                ...
                # (my_set on the legacy backend, a valid SESSION_PID on
                # vyconf) to write into. certbot's renewal timer runs this
                # script standalone, outside of any session. There is no
                # reliable way to predict this in advance, so just try the
                # write and fall back on the ValueError add_cli_node()
                # raises (on both backends) when there is no session to
                # write into.
                try:
                    add_cli_node(['pki', 'ca', f'{AUTOCHAIN_PREFIX}{cert}', 'certificate'],
                                 value=cert_chain_base64, config=pki.conf)
                    Message(f'Add/replace automatically imported CA certificate for "{cert}" ...')
                except ValueError:
                    Warning(f'CA certificate for "{cert}" has changed; run "commit" '
                            'to refresh the automatically imported CA certificate')
                    # Emit a marker op_mode/pki.py's renew_certbot() can pick
                    # up the already-computed value from, and commit it
                    # through a real session, without needing its own
                    # access to the certbot directory.
                    print(f'{AUTOCHAIN_UPDATE_MARKER}{cert}:{cert_chain_base64}')
