...
def apply(snmp):
    ...
    if 'deleted' in snmp:
        ...
        try:
            import os as _os
            _os.unlink('/etc/default/igos-wwan-snmp-traps')
...
def _sync_wwan_snmp_trap_env(snmp):
    """Re-render /etc/default/igos-wwan-snmp-traps from snmp.trap_target.

    Only updates the file if it already exists (i.e. WWAN previously
    enabled it).  Creating it from here would launch the emitter without
    a corresponding WWAN interface, which is undesirable.
    """
    import os as _os
    env_file = '/etc/default/igos-wwan-snmp-traps'
    if not _os.path.exists(env_file):
        return

    targets = snmp.get('trap_target') or {}
    if not targets:
        # trap-target was removed; stop the emitter.
        call('systemctl stop igos-wwan-snmp-traps.service')
        try:
            _os.unlink(env_file)
        except FileNotFoundError:
    ...
    body = (
        ...
        f'IGOS_SNMPTRAP_DEST=udp:{addr}:{port}\n'
